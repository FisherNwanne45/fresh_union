...
{
    $sql = "SELECT SUM(amount) as total FROM transactions WHERE user_id = :user_id AND transaction_type = :type AND created_at >= :start AND created_at <= :end";
...
$sql_transactions = "SELECT amount, transaction_type, trans_type, created_at FROM transactions WHERE user_id = :user_id ORDER BY created_at DESC LIMIT 100";
                            ...
                            <div class="transaction-item">
                                <div class="transaction-info">
                                    ...
                                    <div class="transaction-details">
                                        ...
                                        <span
                                            class="text-muted"><?php echo date('M j, Y', strtotime($transaction['date'])); ?></span>
                                    </div>
                                </div>
                                <div class="transaction-amount">
                                    <span class="amount text-<?= $transaction['type'] === 'income' ? 'success' : 'danger'; ?>">
                                        <?php echo $transaction['type'] === 'income' ? '+' : '-'; ?><?= $currency ?><?php echo number_format($transaction['amount'], 2); ?>
                                    </span>
                                </div>
                            </div>
                        <?php endforeach; ?>
